...
(async () => {
    ...
    await axios.get(wtjUrl, { responseType: 'document' }).then((response) => {
        ...
        (async () => {
            ...
            newLinkText = '<li style="opacity: 1" id="logo-name">WebTigerJython</li><li style="font-size: 20; transform: translateX(50%) translateY(49%); filter: drop-shadow(0px 0px 5px #9738fc); color: #9738fc" id="logo-v2" title="Cloud-Version by Beat Temperli">CLOUD</li>';
            ...
            axios.get('https://cdnjs.cloudflare.com/ajax/libs/ace/1.36.2/ace.js')
                .then(response => fs.writeFileSync(`${wtjFolder}/ace.js`, response.data))
                .catch(console.error).finally(() => console.log('ace.js downloaded')).finally(() => console.log('successfully updated wtj!'));
        });
    });
